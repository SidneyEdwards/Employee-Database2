    ...
    formatListQuestion("employee","employee_id",res),
        ...
        message: "What do you want to update for this employee?",
            ...
            let id = {id: answers.employee_id};
                ...
                case "Both":
                    db.choices.roles().then(res => {
                        inquirer.prompt([formatListQuestion("roles","id",res)]).then(answers => {
                            let newInfo = answers;
                                db.choices.employees().then(res => {
                                inquirer.prompt([formatListQuestion("manager","manager",res)]).then(answers => {
        ...
        db.choices.roles().then(res => {
            inquirer.prompt([
                formatListQuestion("roles","id",res),
                {
                ...
                db.update(table_name, answers, {id: roleId}, callMainPrompt);
                    ...
                    db.update(table_name, answers, {id: roleId}, callMainPrompt);
                        ...
                        db.choices.departments().then(res => {
                            inquirer.prompt([
                            formatListQuestion("departments","department_id",res)
                            ]).then(answers => {
                            db.update(table_name, answers, {id: roleId}, callMainPrompt);
                            });
                            });
